...
# DIRECTORIES
outdir = paste0("AQ_maps/", vrsn, "/annual/", pollutant, "_", stat)
...
model_msrs = matrix(nrow=0, ncol=16) |> as.data.frame()
...
for (y in years){
  ...
  if (pollutant == "NO2" && y < 2018 ){
    message("NO2: skipping ", y)
    next()
  }
  compo_paths = file.path(tifdir, 
                          paste0(paste(pollutant, stat, y, area_types, vrsn, sep = "_"), ".tif")) |> 
    setNames(area_types)
  ...
  if (any(!file.exists(compo_paths))){
    ...
    for (a_type in area_types){
      if (!file.exists(compo_paths[a_type])){
        ...
        model_msrs = rbind(model_msrs, 
                           c(poll=pollutant, stat=stat, year=y, type=a_type, 
  ...
  st = as.Date(paste(y,"01","01",sep = "-"))
  en = (lubridate::ceiling_date(st, "year") - 1 ) |> format("%Y%m%d")
  ...
  if (!file.exists(cog_path_aq)){
    message("Creating ", basename(cog_path_aq))
    
    # merge
    aq_merge = merge_aq_maps(compo_paths, pse_paths, weights, cluster = cl)
    
    # aq tif and cog
    merged_tif_path = sub("RB","merged", compo_paths[1])
    ...
    merged_pdf_paths = file.path(pdfdir_merged, 
                                 sub(".tif", ".pdf", c(
                                   basename(merged_tif_path), 
                                   paste0("PSE_", basename(merged_tif_path)))))
    ...
    if ("pred_se" %in% names(aq_merge)){
      cog_path_pse = file.path(cogdir, paste0("air_quality.pse_", tolower(pollutant), perc,"_1km_s_", 
                                              format(st, "%Y%m%d"), "_", en, "_eu_epsg.3035_", vrsn, ".tif"))
      pse_path = paste0(tifdir, "/PSE_", basename(merged_tif_path)) 
    ...
    model_msrs = rbind(model_msrs, 
                       c(poll=pollutant, stat=stat, year=y, type="all", 
