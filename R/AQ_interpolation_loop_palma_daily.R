...
args = commandArgs(trailingOnly=TRUE) # 1. variable of interest | 2. percentile or mean | 3. n cores
...
re = T              # reverse temporal order?
years = c(2015:2023)
if (pollutant == "NO2") years = c(2018:max(years))
days = 1:366
if (re) {
  years = rev(years)
  days = rev(days)
...
# DIRECTORIES
outdir = paste0("AQ_maps/", vrsn, "/daily/", pollutant, "_", stat)
#tifdir = file.path(outdir, "tif")
#pdfdir = file.path(outdir, "pdf", "components")
pdfdir_merged = file.path(outdir, "pdf")
cogdir = file.path(outdir, "cog")
create_dir_if(c(pdfdir_merged, cogdir), recursive=T)

grid_resolution = 10           # ifelse(pollutant=="O3", 10, 1)
dem = readRDS(paste0(
  "supplementary/static/COP-DEM/COP_DEM_Europe_mainland_",
  grid_resolution, "km_mask_epsg3035.rds"))

weights = readRDS(paste0("supplementary/static/merge_weights/merge_weights_",
                         grid_resolution, "km.rds"))

perc = switch(stat,
              "perc" = ifelse(pollutant=="PM10", "_90.41", NULL),
              "max" = ifelse(pollutant=="O3","_max8h.avg", NULL),
              NULL)
...
for (y in years) {
  ...
  pdfdir_merged_y = file.path(pdfdir_merged, y)
  create_dir_if(c(cogdir_y, pdfdir_merged_y))
  ...
  for (d in days) {
    ...
    st = as.Date(d-1, origin = paste(y, "01", "01", sep = "-")) |> format("%Y%m%d")
    en = st
    
    cog_path_aq = file.path(cogdir_y, paste0("air_quality.", tolower(pollutant), perc, "_", grid_resolution, "km_s_", 
    ...
    if (!file.exists(cog_path_aq)) {
      ...
      # station data
      aq = load_aq(pollutant, stat, y=y, d=d) 
      
      # handle leap years
      if (nrow(aq) == 0) {
        message("No AQ measurements for year ", y, " - doy ", d, " ---> skipping to next day.")
        next()
      } 
      ...
      for (a_type in area_types) {
        ...
        #message("test extract 2")
        #print(st_extract(aq_cov_t, aq_t) |> summary())
        ...
        compo_list[[a_type]] = result  # [c("aq_pred","pred_se")]
        
        #message("6. write - measure - plot...")
        model_msrs = rbind(model_msrs, 
                           c(poll=pollutant, stat=stat, year=y, doy=d, type=a_type, 
      ...
      # merge
      aq_merge = merge_aq_maps_list(compo_list, weights, cluster = cl, verbose = F)
      ...
      # pse tif and cog
      cog_path_pse = file.path(cogdir_y, paste0("air_quality.pse_", tolower(pollutant), perc, "_", 
                                                grid_resolution, "km_s_", st, "_", en, "_eu_epsg.3035_", vrsn, ".tif"))
      ...
      merged_pdf_paths = file.path(pdfdir_merged_y, 
                                   sub(".tif", ".pdf", c(basename(cog_path_aq), basename(cog_path_pse))))
      ...
      model_msrs = rbind(model_msrs, 
                         c(poll=pollutant, stat=stat, year=y, doy=d, type="all", 
      ...
      parallel::clusterEvalQ(cl, {gc(verbose=F)})
      gc(verbose=F)
      cat(tictoc::toc(quiet = T)$callback_msg, "\n")
    }
  }
}
